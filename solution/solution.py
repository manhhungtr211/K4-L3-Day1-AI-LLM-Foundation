...
def compare_models(prompt: str) -> dict:
    ...
    """
    model_result = call_openai(prompt)
    model_mini_result = call_openai_mini(prompt)
    cost = (len(model_result[0].split()) / 0.75) / 1000 * PRICING_PER_1K_TOKENS["gpt-4o"]["output"]

    return {
        "gpt4o_response": model_result[0],
        "mini_response": model_mini_result[0],
        "gpt4o_latency": model_result[1],
        "mini_latency": model_mini_result[1],
...
def run_assistant(
    ...
    while True:
        ...
        for chunk in stream:
            delta = chunk.choices[0].delta.content or ""
            print(delta, end="", flush=True)
            collect_chunks.append(delta)
        response = "".join(collect_chunks)
        num_turns += 1
        total_tokens += count_tokens(messages) + count_tokens([response])
        total_cost += estimate_cost(messages,response)["total_cost"]
